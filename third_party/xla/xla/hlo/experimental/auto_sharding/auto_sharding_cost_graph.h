...
struct EdgeReshardingCost {
  double communication_cost_ = 0;
  double memory_cost_ = 0;

  EdgeReshardingCost() : communication_cost_(0), memory_cost_(0) {}

  EdgeReshardingCost(double communication_cost, double memory_cost)
      : communication_cost_(communication_cost), memory_cost_(memory_cost) {}

  EdgeReshardingCost operator+(const EdgeReshardingCost& other) const {
    return EdgeReshardingCost(other.communication_cost_ + communication_cost_,
                              other.memory_cost_ + memory_cost_);
  }

  std::string ToString() {
    return absl::StrCat("{communication_cost=", communication_cost_,
                        ", memory_cost=", memory_cost_, "}");
  }
};
        ...
        if (dst_strategy.communication_cost > 0) {
          ...
          if (it != src_strategy_name_to_idx_map.end()) {
            ...
            CHECK_LE(std::abs(src_strategy.communication_cost -
                     ...
                     1e-6);
            edge_cost(it->second, i).communication_cost_ =
    ...
    if (node_lens_[dst] == node_lens_[src]) {
      ...
      for (NodeStrategyIdx i = 0; i < node_lens_[dst]; ++i) {
        ...
        for (NodeStrategyIdx j = 0; j < node_lens_[src]; ++j) {
          keys.push_back({edge_cost(i, j).communication_cost_, -j});
        }
      ...
      if (adj == dst) {
        for (NodeStrategyIdx i = 0; i < node_lens_[dst]; ++i) {
          extra_node_costs_[dst][i] +=
              edge_cost(i, reindexing[i]).communication_cost_;
        }
      } else {
