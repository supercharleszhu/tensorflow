...
// Returns true if a pipelineable collective-permute. As a simple heuristic,
...
// Decomposes a collective-permute and add frontend attribute to record
...
// cycle for pipelining. If the two collective-permutes for a cycle, returns
...
absl::StatusOr<bool> CollectivePermuteDecomposer::Run(
  ...
  for (auto iter = all_computations.rbegin(); iter != all_computations.rend();
    ...
    for (HloCollectivePermuteInstruction* cp : cps_to_decompose) {
      VLOG(0) << "Decomposing collective-permute: " << cp->ToString();
